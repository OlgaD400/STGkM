...
class STGKM:
    ...
    def __init__(
        ...
        assert (
            num_clusters <= self.num_vertices
        ), "Cannot run STGkM with more clusters than vertices."

        assert self.drift_time_window > 0, "Center connectivity must be at least 1."
        assert self.max_drift > 0, "Max drift must be at least 1."
        ...
        if self.tie_breaker is True:
            ...
            self.full_assignments = np.zeros(
                (self.timesteps * self.k, self.num_vertices)
            )
    ...
    def assign_points(
        self, distance_matrix: np.ndarray, centers: np.ndarray
    ) -> np.ndarray:
        """
        ...
        membership_matrix = np.where(
            center_distances == min_center_distances_matrix, 1, 0
        )
        ...
        if self.tie_breaker is True:
            membership = np.array(
                [
                    random.choice(np.where(membership_matrix[:, col] > 0)[0])
    ...
    def choose_centers(
        self, distance_matrix: np.ndarray, membership: np.ndarray, centers: np.ndarray
    ) -> np.ndarray:
        """
        ...
        if self.tie_breaker is False:
            ...
            membership = np.array(
                [
                    random.choice(np.where(membership[:, col] > 0)[0])
                    for col in range(self.num_vertices)
                ]
            )
        ...
        for cluster in range(self.k):
            ...
            if len(member_distances) > 0:
                ...
                # centers[cluster] in minimal_members:
                if centers[cluster] in members[minimal_members]:
                    # if the current center is the min distance away from
                    # other points in the cluster, it doesn't change
    ...
    def calculate_intra_cluster_distance(
        ...
        intra_cluster_sum = np.sum(
            np.where(membership == 1, distance_matrix[centers, :], 0)
        )
    ...
    def first_kmeans(self, distance_matrix: np.ndarray):
        ...
        for iterations in range(self.iter):
            membership = self.assign_points(
                distance_matrix=init_matrix, centers=curr_centers
            )

            new_centers = self.choose_centers(
                distance_matrix=init_matrix, membership=membership, centers=curr_centers
            )
            ...
            if (new_centers == curr_centers).all():
                ...
                membership = self.assign_points(
                    distance_matrix=init_matrix, centers=new_centers
                )
    ...
    def find_center_connections(
        self, current_centers: np.ndarray, distance_matrix: np.ndarray, time: int
    ) -> List[List[int]]:
        """
        ...
        drift_time_slices = distance_matrix[(time - self.drift_time_window) : time]
        # The sum across the "center connectivity" previous distance matrices can be no
        # larger than target sum
        target_sum = self.max_drift * min(time, self.drift_time_window)

        center_connections = [
            np.where(np.sum(drift_time_slices[:, center], axis=0) <= target_sum)[0]
    ...
    def next_assignment_proxy(
        self, current_centers, distance_matrix: np.ndarray, time: int
    ):
        """
        ...
        center_connections = self.find_center_connections(
            current_centers=current_centers, distance_matrix=distance_matrix, time=time
        )
        ...
        current_membership = self.assign_points(
            distance_matrix=current_distance, centers=current_centers
        )
            ...
            if len(center_k_possibilities) > 1:
                ...
                for possibility in center_k_possibilities:
                    ...
                    if len(set(changing_centers)) == self.k:
                        membership = self.assign_points(
                            distance_matrix=current_distance, centers=changing_centers
                        )
    ...
    def next_assignment(
        ...
        for center_combination in itertools.product(*center_connections):
            ...
            if len(set(center_combination)) == self.k:
                membership = self.assign_points(
                    distance_matrix=current_distance, centers=center_combination
                )
    ...
    def run_stgkm(self, method: Literal["full", "proxy"] = "full"):
        ...
        Args:
            method (Literal['full', 'proxy']): Whether to run STGkM with optimal
                or approximate assignment.
        """
        print("Running stgkm")
        ...
        for time in range(1, self.timesteps):
            ...
            if self.tie_breaker is True:
                ...
                self.full_assignments[
                    time * (self.k) : (time + 1) * self.k, :
                ] = new_members

            current_centers = list(new_centers).copy()

        # Find long term clusters based on total assignment history
        self.ltc = agglomerative_clustering(
            weights=self.full_assignments.T, num_clusters=self.k
        )
